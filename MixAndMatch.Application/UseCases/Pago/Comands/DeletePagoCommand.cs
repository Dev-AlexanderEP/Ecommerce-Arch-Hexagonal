...
{
    public async Task<ApiResponse<bool>> Handle(DeletePagoCommand request, CancellationToken cancellationToken)
    {
        var entity = await _uow.Repository<PagoEntity>().GetById(request.Id);
        if (entity is null)
        {
            return ApiResponse<bool>.Fail($"Pago no encontrado para id {request.Id}.");
        }

        await _uow.Repository<PagoEntity>().Delete(request.Id);
        await _uow.Complete();

        return ApiResponse<bool>.Ok(true, "Pago eliminado correctamente.");
    }
}
